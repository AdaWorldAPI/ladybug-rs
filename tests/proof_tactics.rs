...
fn tactics_t01_recursive_expansion_converges() {
    ...
    assert!(trace.depth() > 0, "Should produce at least one expansion step");
...
fn tactics_t03_debate_improves_truth() {
    ...
    let pro_args = vec![
        Argument {
            fingerprint: Fingerprint::from_content("symmetry_approach").as_raw().clone(),
    ...
    assert!(outcome.final_truth.confidence > initial.confidence,
...
fn tactics_t04_reverse_causal_trace() {
    ...
    search.store_intervention(
        state.as_raw(), action.as_raw(), outcome.as_raw(), 1.0
    );
    ...
    assert!(trace.depth() >= 1,
        "Should find at least one causal step, got depth {}", trace.depth());

    // The root cause should match the original state
    if let Some(root) = trace.root_cause() {
        assert_eq!(root, state.as_raw(),
...
fn tactics_t07_adversarial_detects_weakness() {
    ...
    assert_eq!(challenges.len(), 5,
        "Should generate exactly 5 challenges, got {}", challenges.len());
...
fn tactics_t10_metacog_tracks_calibration() {
    ...
    assert!(good_brier < 0.1,
        "Well-calibrated predictions should have low Brier: {:.3}", good_brier);
...
fn tactics_t11_contradiction_detection() {
    ...
    assert!(coherence < 1.0,
        "Contradictions should reduce coherence: {:.3}", coherence);
}
...
fn tactics_t15_crp_distribution() {
    ...
    // Percentile ordering
    assert!(dist.p25 <= dist.p50, "p25={:.1} should be ≤ p50={:.1}", dist.p25, dist.p50);
    assert!(dist.p50 <= dist.p75, "p50={:.1} should be ≤ p75={:.1}", dist.p50, dist.p75);
    assert!(dist.p75 <= dist.p95, "p75={:.1} should be ≤ p95={:.1}", dist.p75, dist.p95);
    assert!(dist.p95 <= dist.p99, "p95={:.1} should be ≤ p99={:.1}", dist.p95, dist.p99);

    // For random fingerprints: mean should be near FINGERPRINT_BITS/2 (≈8192)
    assert!((dist.mu - 8192.0).abs() < 500.0,
        "Mean {:.1} should be near 8192 for random fingerprints", dist.mu);

    // Z-score of mean should be ~0
    let z_at_mean = dist.z_score(dist.mu);
    assert!(z_at_mean.abs() < 0.01,
        "Z-score at mean should be ~0, got {:.3}", z_at_mean);
}
...
fn tactics_t20_shadow_consensus() {
    ...
    assert!(comparison.agreement,
        "Identical results should agree");
    assert!(comparison.divergence < 0.001,
        "Identical results should have ~0 divergence: {:.3}", comparison.divergence);
    ...
    assert!(!comparison2.agreement,
        "Different results should NOT agree");

    // Agreement rate should reflect mixed history
    assert!(processor.agreement_rate() < 1.0,
        "Mixed history should show <100% agreement: {:.3}", processor.agreement_rate());
...
fn tactics_t24_fusion_quality_exact() {
    for i in 0..100 {
        ...
        assert_eq!(dist_a, 0.0,
            "Recovery of A should be exact at i={}, got {}", i, dist_a);
        assert_eq!(dist_b, 0.0,
            "Recovery of B should be exact at i={}, got {}", i, dist_b);
    }
}
...
fn tactics_t25_hamming_normal() {
    ...
    assert!((mean - mu).abs() / mu < 0.01,
        "Mean {:.1} should be within 1% of theoretical {:.1}", mean, mu);

    // Sigma within 5% of theoretical
    assert!((observed_sigma - sigma).abs() / sigma < 0.05,
        "Sigma {:.1} should be within 5% of theoretical {:.1}", observed_sigma, sigma);
}
...
fn tactics_t28_temporal_granger() {
    ...
    let series_a: Vec<[u64; WORDS]> = (0..20)
        .map(|i| Fingerprint::from_content(&format!("granger_a_{}", i)).as_raw().clone())
        .collect();
...
fn tactics_t31_counterfactual_divergence() {
    ...
    assert!(w1.divergence > 0.3, "World 1 should diverge: {:.3}", w1.divergence);
    assert!(w2.divergence > 0.3, "World 2 should diverge: {:.3}", w2.divergence);

    // And from each other
    let mutual = worlds_differ(&w1, &w2);
    assert!(mutual > 0.3, "Worlds should differ from each other: {:.3}", mutual);
}
...
fn tactics_t34_cross_domain_fusion() {
    ...
    assert_eq!(recovered_text.as_raw(), text_fp.as_raw(),
        "Text domain should be exactly recoverable");
    assert_eq!(recovered_number.as_raw(), number_fp.as_raw(),
        "Number domain should be exactly recoverable");
    assert_eq!(recovered_graph.as_raw(), graph_fp.as_raw(),
