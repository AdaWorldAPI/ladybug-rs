...
fn foundation_f1_berry_esseen_16k() {
    ...
    assert!((variance - d / 4.0).abs() / (d / 4.0) < 0.05,
        "Variance {:.1} should be within 5% of theoretical {:.1}", variance, d / 4.0);
}
...
fn foundation_f2_fisher_sufficiency() {
    ...
    assert!(mu_diff < 0.05,
        ...
        crp_100.mu, crp_500.mu, mu_diff);
    assert!(sigma_diff < 0.15,
        "σ should converge: 100-sample={:.1}, 500-sample={:.1} (diff={:.3})",
        crp_100.sigma, crp_500.sigma, sigma_diff);
}
...
fn foundation_f3b_xor_commutativity_associativity() {
    for i in 0..100 {
        ...
        assert_eq!(a.bind(&b).as_raw(), b.bind(&a).as_raw(),
            "XOR commutativity violated at i={}", i);

        // Associativity: (A ⊗ B) ⊗ C = A ⊗ (B ⊗ C)
        let left = a.bind(&b).bind(&c);
        let right = a.bind(&b.bind(&c));
        assert_eq!(left.as_raw(), right.as_raw(),
            "XOR associativity violated at i={}", i);
    }
}
...
fn foundation_f5b_calibrated_thresholds() {
    ...
    assert!(thresholds.excite < thresholds.inhibit,
        "excite={} should be < inhibit={}", thresholds.excite, thresholds.inhibit);
...
fn foundation_f6b_revision_commutativity() {
    for i in 0..100 {
        ...
        assert!((ab.frequency - ba.frequency).abs() < 0.001,
            "Revision should be commutative: rev(A,B).f={:.4}, rev(B,A).f={:.4} at i={}",
            ab.frequency, ba.frequency, i);
        assert!((ab.confidence - ba.confidence).abs() < 0.001,
            "Revision should be commutative: rev(A,B).c={:.4}, rev(B,A).c={:.4} at i={}",
            ab.confidence, ba.confidence, i);
    }
}
...
fn foundation_f7b_fusion_quality_exact() {
    for i in 0..100 {
        ...
        assert_eq!(dist_a, 0.0, "Recovery of A should be exact at i={}, got {}", i, dist_a);
        assert_eq!(dist_b, 0.0, "Recovery of B should be exact at i={}, got {}", i, dist_b);
    }
}
...
fn foundation_f7c_multi_fusion_exact() {
    ...
    assert_eq!(max_dist, 0.0,
        "Multi-way bind should be exactly recoverable, got {}", max_dist);
}
