...
impl CodebookEntry {
    pub fn new() -> Self {
        Self {
            centroid: Fingerprint::from_seed(rand::random()),
            count: 0,
            accumulator: vec![0u32; N],
            chunk_ids: Vec::new(),
        }
    }
    ...
    pub fn new() -> Self {
        Self {
            cells: Box::new(std::array::from_fn(|_| {
                std::array::from_fn(|_| std::array::from_fn(|_| CodebookEntry::new()))
            })),
            total_chunks: 0,
            total_bytes_raw: 0,
            cells_used: 0,
...
impl BTRProcella {
    ...
    pub fn update(&mut self, s: (f64, f64, f64), a: RLAction, r: f64, ns: (f64, f64, f64)) {
        ...
        let ai = match a {
            RLAction::IncreaseResidual => 0,
            RLAction::DecreaseResidual => 1,
            RLAction::Refine => 2,
            RLAction::Hold => 3,
        };
        let nm = self
    ...
    pub fn apply(&mut self, a: RLAction, cb: &mut CrystalCodebook) {
        match a {
            ...
            RLAction::DecreaseResidual => {
                self.residual_k = self.residual_k.saturating_sub(8).max(8)
            }
...
impl LangExtractor {
    ...
    pub fn extract(&self, source: &str) -> Vec<Chunk> {
        ...
        for line in source.lines() {
            let boundary = line.trim().starts_with("fn ")
...
impl ProgrammingSavant {
    pub fn new(ctx: usize) -> Self {
        Self {
            extractor: LangExtractor::new(512),
            codebook: CrystalCodebook::new(),
            rl: BTRProcella::new(),
    ...
    pub fn train(&mut self, queries: &[(&str, &str)]) {
        let cr =
            self.codebook.total_bytes_raw as f64 / self.codebook.compressed_bytes().max(1) as f64;
        ...
        self.rl
            .update(state, action, reward, (cr * 1.01, dist * 0.99, acc));
    }
}
...
fn _example_main() {
    ...
    let (raw, comp, dist, chunks, cells) = (
        savant.codebook.total_bytes_raw,
        savant.codebook.compressed_bytes(),
        savant.codebook.avg_distortion,
        savant.codebook.total_chunks,
        savant.codebook.cells_used,
    );
    for ep in 0..10 {
        savant.train(&test);
        if ep % 2 == 0 {
            println!(
                "  Ep {}: reward={:.3}, residual_k={}",
                ep,
                savant.rl.total_reward / (ep + 1) as f64,
                savant.rl.residual_k
            );
        }
    }
    println!();
    ...
    println!("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━");
    println!(
        "  {} tokens → {} chunks → {} cells → {} KB compressed",
        tokens,
        cb_chunks,
        cb_cells,
        cb_comp / 1024
    ...
    println!(
        "  Compression: {:.1}x",
        cb_raw as f64 / cb_comp.max(1) as f64
    );
    println!(
        "  Query: <1ms for 8K context from {}K codebase",
        tokens / 1000
    );
    println!();
}
...
fn generate(n: usize) -> String {
    ...
    let tpl = [
        ("process_data", "data: &[u8]"),
        ("authenticate", "user: &str, pass: &str"),
        ("connect_database", "config: &Config"),
        ("cache_lookup", "key: &str"),
        ("cache_insert", "key: &str, val: &[u8]"),
        ("validate", "input: &str"),
    ];
    for i in 0..n {
        let (name, args) = tpl[i % tpl.len()];
        s.push_str(&format!(
            "pub fn {}_v{}({}) -> Result<(), Error> {{\n",
            name,
            i / tpl.len(),
            args
        ));
