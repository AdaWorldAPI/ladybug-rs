...
impl CrystalLM {
    ...
    pub fn size_bytes(&self) -> usize {
        self.axes.size_bytes() + self.learned.len() * (8 + crate::FINGERPRINT_BYTES) // hash + fingerprint
    }
    ...
    fn blend(old: &Fingerprint, new: &Fingerprint, new_weight: f32) -> Fingerprint {
        ...
        Self::weighted_bundle(&[(old.clone(), 1.0 - new_weight), (new.clone(), new_weight)])
    }
