    ...
    pub fn from_text_hash(text: &str) -> Self {
        ...
        for (i, w) in data.iter_mut().enumerate() {
            ...
            for chunk in bytes.windows(3) {
                let trigram = (chunk[0] as u64) | ((chunk[1] as u64) << 8) | ((chunk[2] as u64) << 16);
...
impl ConceptCAM {
    ...
    fn find_nearest(&self, fp: &Fingerprint) -> (usize, u32, bool) {
        ...
        for (i, slot) in self.clusters.iter().enumerate() {
            if slot.examples.is_empty() { continue; }
            let dist = fp.hamming(&slot.fingerprint);
            if dist < best_dist {
                best_dist = dist;
                best_idx = i;
    ...
    pub fn stats(&self) -> CAMStats {
        let archetype_populated = self.archetypes.iter()
            .filter(|s| !s.examples.is_empty())
            .count();
        let clusters_populated = self.clusters.iter()
            .filter(|s| !s.examples.is_empty())
            .count();
...
fn main() {
    ...
    // NARS patterns
    let nars_corpus = vec![
        ("bird is-a animal".to_string(), Some(ConceptType::Inheritance)),
        ("penguin is-a bird".to_string(), Some(ConceptType::Inheritance)),
        ("mammal is-a animal".to_string(), Some(ConceptType::Inheritance)),
        ("dog similar-to wolf".to_string(), Some(ConceptType::Similarity)),
        ("cat resembles tiger".to_string(), Some(ConceptType::Similarity)),
        ("rain implies wet".to_string(), Some(ConceptType::Implication)),
    ...
    // SPO relations
    let spo_corpus = vec![
        ("function contains loop".to_string(), Some(ConceptType::Contains)),
        ("class becomes instance".to_string(), Some(ConceptType::Becomes)),
        ("theory refines hypothesis".to_string(), Some(ConceptType::Refines)),
        ("evidence grounds belief".to_string(), Some(ConceptType::Grounds)),
        ("model abstracts reality".to_string(), Some(ConceptType::Abstracts)),
        ("claim contradicts evidence".to_string(), Some(ConceptType::Contradicts)),
    ];
    
    // Qualia patterns
    let qualia_corpus = vec![
        ("warm fuzzy feeling comfort".to_string(), Some(ConceptType::Felt)),
        ("excitement energy enthusiasm".to_string(), Some(ConceptType::Arousal)),
        ("pleasant positive good".to_string(), Some(ConceptType::Valence)),
        ("uncertain conflicted torn".to_string(), Some(ConceptType::Tension)),
    ];
    ...
    println!("  Archetype slots populated: {} / {}", stats.archetype_populated, stats.archetype_slots);
    println!("  Cluster slots populated: {} / {}", stats.clusters_populated, stats.cluster_slots);
    ...
    println!();
    println!("  {} lookups in {:.3}ms = {:.1}µs per lookup",
             test_queries.len(),
    ...
    let stats = cam.stats();
    println!("  10K lookups in {:.2}ms", bench_time.as_secs_f64() * 1000.0);
    println!("  Throughput: {:.0} lookups/sec", 10_000.0 / bench_time.as_secs_f64());
