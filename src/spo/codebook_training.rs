...
impl KeywordWeights {
    ...
    fn infer_role(prime: &str) -> Option<String> {
        ...
        if [
            "WANT", "KNOW", "THINK", "FEEL", "SEE", "HEAR", "DO", "HAPPEN", "MOVE",
        ]
        .contains(&prime)
        {
...
impl CodebookTrainer {
    ...
    pub fn generate_pairs_from_corpus(texts: &[String], pairs_per_text: usize) -> TrainingBatch {
        ...
        let fps: Vec<Fingerprint> = texts.iter().map(|t| Fingerprint::from_content(t)).collect();

        // For each text, find top-k most similar and create pairs
        for i in 0..texts.len() {
            let mut similarities: Vec<(usize, f32)> = fps
                .iter()
                .enumerate()
                .filter(|(j, _)| *j != i)
    ...
    pub fn learn_concepts(&mut self, texts: &[String], similarity_threshold: f32) {
        // Encode all texts
        let fps: Vec<Fingerprint> = texts.iter().map(|t| self.encode(t)).collect();
        ...
        for (idx, cluster) in clusters.iter().enumerate() {
            let cluster_fps: Vec<Fingerprint> = cluster.iter().map(|&i| fps[i].clone()).collect();

            let name = format!(
                "LEARNED_{}",
                self.substrate.codebook.vocabulary_size() + idx
            );
            let confidence = (cluster.len() as f32 / texts.len() as f32).min(0.9);

            self.substrate
                .codebook
                .learn_concept(&name, &cluster_fps, confidence);
        }
    }
    ...
    pub fn export_weights(&self) -> HashMap<String, Vec<(String, f32)>> {
        ...
        for (keyword, primes) in &self.weights.mappings {
            let weights: Vec<(String, f32)> = primes
                .iter()
...
mod tests {
    ...
    fn test_concept_learning() {
        ...
        println!(
            "Vocab size: {} -> {}",
            initial_vocab,
            trainer.substrate.codebook.vocabulary_size()
        );
    }
}
