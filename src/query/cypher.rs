    ...
    fn transpile_simple_match(query: &CypherQuery, pattern: &Pattern) -> Result<String> {
            ...
            match element {
                PatternElement::Node(node) => {
                    ...
                    if !node.labels.is_empty() {
                        where_parts.push(format!("{}.label = '{}'", alias, node.labels[0]));
                    }
        ...
        if let Some(ref order) = query.order_by {
            let order_sql = order
                ...
                .iter()
                .map(|item| {
                    let dir = if item.direction == SortDirection::Desc {
                        "DESC"
                    } else {
                        "ASC"
                    };
...
"#,
    ...
    fn extract_path_pattern(
        pattern: &Pattern,
    ) -> Result<(NodePattern, EdgePattern, Option<NodePattern>)> {
    ...
    fn condition_to_sql(cond: &Condition) -> String {
        match cond {
            ...
            Condition::And(left, right) => {
                format!(
                    "({}) AND ({})",
                    Self::condition_to_sql(left),
                    Self::condition_to_sql(right)
                )
            }
