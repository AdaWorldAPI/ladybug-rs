...
impl QualiaField {
    ...
    fn compute_valence(text: &str) -> f32 {
        let positive = ["love", "joy", "bright", "beautiful", "wonder", "delight", 
                       "happy", "good", "great", "excellent", "amazing", "wonderful"];
        let negative = ["hate", "sorrow", "dark", "terrible", "cruel", "pain", 
                       "death", "bad", "awful", "horrible", "sad", "angry"];
    ...
    fn compute_arousal(text: &str) -> f32 {
        let high = ["!", "suddenly", "burst", "cry", "passion", "fire", "explode",
                   "intense", "extreme", "overwhelming", "ecstatic", "furious"];
        let low = ["calm", "peaceful", "quiet", "gentle", "soft", "serene",
                  "tranquil", "relaxed", "still", "silent"];
    ...
    fn compute_dominance(text: &str) -> f32 {
        let dominant = ["control", "power", "command", "lead", "decide", "force",
                       "dominate", "rule", "authority", "master"];
        let submissive = ["submit", "obey", "follow", "serve", "yield", "surrender",
    ...
    fn compute_intimacy(text: &str) -> f32 {
        let intimate = ["heart", "soul", "love", "dear", "soft", "gentle", "whisper",
                       "touch", "embrace", "kiss", "close", "together", "us", "we"];
    ...
    fn compute_certainty(text: &str) -> f32 {
        let certain = ["definitely", "certainly", "absolutely", "clearly", "obviously",
                      "undoubtedly", "surely", "must", "always", "never"];
        let uncertain = ["maybe", "perhaps", "possibly", "might", "could", "seems",
                        "appears", "uncertain", "unclear", "sometimes"];
    ...
    fn compute_urgency(text: &str) -> f32 {
        let urgent = ["now", "immediately", "urgent", "hurry", "quick", "fast",
                     "asap", "deadline", "emergency", "critical", "rush"];
    ...
    fn compute_depth(text: &str) -> f32 {
        let deep = ["fundamental", "essence", "core", "underlying", "profound",
                   "deep", "meaning", "philosophy", "metaphysical", "existential"];
        let surface = ["surface", "obvious", "simple", "basic", "straightforward",
                      "literal", "plain", "clear"];
    ...
    fn compute_novelty(text: &str) -> f32 {
        let novel = ["new", "novel", "unique", "original", "innovative", "fresh",
                    "unprecedented", "surprising", "unexpected", "discovery"];
        let familiar = ["usual", "typical", "common", "normal", "regular",
                       "routine", "standard", "ordinary", "familiar"];
    ...
    fn compute_complexity(text: &str) -> f32 {
        let complex = ["complex", "complicated", "intricate", "sophisticated",
                      "nuanced", "multifaceted", "layered", "elaborate"];
        let simple = ["simple", "easy", "basic", "straightforward", "clear",
                     "plain", "obvious", "direct"];
    ...
    fn compute_coherence(text: &str) -> f32 {
        let coherent = ["therefore", "thus", "because", "so", "hence", "consequently",
                       "connected", "unified", "consistent", "follows"];
        let fragmented = ["but", "however", "although", "despite", "yet",
                         "random", "scattered", "disconnected"];
    ...
    fn compute_salience(text: &str) -> f32 {
        let salient = ["important", "crucial", "critical", "essential", "key",
                      "vital", "significant", "notable", "remarkable", "!"];
    ...
    fn compute_aesthetic(text: &str) -> f32 {
        let beautiful = ["beautiful", "elegant", "graceful", "lovely", "stunning",
                        "gorgeous", "artistic", "aesthetic", "refined", "exquisite"];
    ...
    fn compute_moral(text: &str) -> f32 {
        let good_moral = ["right", "ethical", "moral", "just", "fair", "honest",
                         "virtuous", "noble", "honorable", "good"];
        let bad_moral = ["wrong", "immoral", "unjust", "unfair", "dishonest",
                        "evil", "corrupt", "wicked"];
    ...
    fn compute_social(text: &str) -> f32 {
        let social = ["people", "society", "community", "public", "social",
                     "everyone", "we", "us", "together", "shared"];
        let private = ["private", "personal", "individual", "alone", "solitary",
                      "secret", "hidden"];
    ...
    fn compute_temporal_span(text: &str) -> f32 {
        let eternal = ["forever", "eternal", "always", "never-ending", "timeless",
                      "permanent", "everlasting", "infinite"];
        let momentary = ["moment", "instant", "brief", "temporary", "fleeting",
                        "now", "today", "currently"];
    ...
    fn compute_spatial_span(text: &str) -> f32 {
        let universal = ["universe", "world", "global", "everywhere", "all",
                        "universal", "cosmic", "infinite", "vast"];
        let local = ["here", "local", "nearby", "this place", "small",
                    "particular", "specific"];
    ...
    fn compute_concreteness(text: &str) -> f32 {
        let concrete = ["physical", "tangible", "material", "solid", "real",
                       "object", "thing", "body", "hand", "see", "touch"];
        let abstract_words = ["concept", "idea", "abstract", "theoretical",
                             "principle", "notion", "thought", "mental"];
    ...
    fn compute_intentionality(text: &str) -> f32 {
        let purposeful = ["intend", "purpose", "goal", "aim", "want", "plan",
                         "decide", "choose", "will", "deliberately"];
        let aimless = ["random", "accidental", "happen", "chance", "aimless",
                      "wandering", "drift"];
    ...
    pub fn similarity(&self, other: &Self) -> f32 {
        let dot: f32 = self.coordinates.iter()
            .zip(other.coordinates.iter())
            .map(|(a, b)| a * b)
            .sum();
