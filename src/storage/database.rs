...
impl Database {
    ...
    pub fn resonate(
        ...
        engine
            .search_threshold(fingerprint, threshold, limit)
            .into_iter()
    ...
    pub async fn causes(&self, from_id: &str, to_id: &str, amplification: f32) -> Result<()> {
        let edge = EdgeRecord::new(from_id, to_id, "CAUSES").with_amplification(amplification);
    ...
    pub async fn amplifies(&self, from_id: &str, to_id: &str, factor: f32) -> Result<()> {
        let edge = EdgeRecord::new(from_id, to_id, "AMPLIFIES").with_amplification(factor);
        self.add_edge(edge).await
    }
