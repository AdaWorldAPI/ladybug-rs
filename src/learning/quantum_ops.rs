...
impl QuantumOp for BindOp {
    ...
    fn is_hermitian(&self) -> bool { true }  // XOR is self-inverse
    
    fn adjoint(&self) -> Box<dyn QuantumOp> {
        Box::new(BindOp { operand: self.operand.clone() })
    }
}
...
impl QuantumOp for PermuteOp {
    ...
    fn is_hermitian(&self) -> bool { false }  // Unless shift = 0
...
impl QuantumOp for ProjectOp {
    ...
    fn is_hermitian(&self) -> bool { true }  // P² = P
    
    fn adjoint(&self) -> Box<dyn QuantumOp> {
        Box::new(ProjectOp { mask: self.mask.clone() })
    }
}
...
impl QuantumOp for HadamardOp {
    ...
    fn is_hermitian(&self) -> bool { true }  // H² = I
...
impl QuantumOp for MeasureOp {
    ...
    fn adjoint(&self) -> Box<dyn QuantumOp> {
        Box::new(MeasureOp { eigenstates: self.eigenstates.clone() })
    }
}
...
impl ActrRetrievalOp {
    pub fn new(chunks: Vec<Fingerprint>, activations: Vec<f32>, threshold: f32) -> Self {
        Self { chunks, activations, threshold }
    }
}
...
impl QuantumOp for RungLadderOp {
    ...
    fn adjoint(&self) -> Box<dyn QuantumOp> {
        Box::new(RungLadderOp { direction: -self.direction })
    }
}
...
impl QuantumOp for SumOp {
    fn apply(&self, state: &Fingerprint) -> Fingerprint {
        ...
        let results: Vec<Fingerprint> = self.ops.iter()
            .map(|op| op.apply(state))
            .collect();
    ...
    fn adjoint(&self) -> Box<dyn QuantumOp> {
        ...
        Box::new(SumOp { ops: adj_ops, weights: self.weights.clone() })
    }
}
...
impl TensorOp {
    pub fn new(
        ...
        Self { op_a, op_b, range_a, range_b }
    }
}
